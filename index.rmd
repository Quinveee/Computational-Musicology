...
multi_plot_1 <- ggplot(corpus_with_title, aes(x = liveness, acousticness, color = speechiness, size = speechiness, label = title)) +
...
slowdeath <-
  get_tidy_audio_analysis("5Hf2h59YLInKlic7ooWZVd") |>
  select(segments) |>
  unnest(segments) |>
  select(start, duration, pitches)
