...
```{r, echo=FALSE}
appendix <- corpus %>%
  select(track.album.name, track.name, track.album.release_date)

# # a <- appendix[2, "track.album.id"]%>% pull(track.album.id) nrow(appendix)
# for( i in 1:1){
#   a <- unlist(appendix[i, "track.artists"])
#   print(gregexec("([[:alpha:]]+)([[:digit:]]+)", a))
#   appendix[i, "artist"] <- appendix[i, "track.artists"][[1]]
# }
# 
# 
# glimpse(appendix)
datatable(appendix, height = 200, style = "auto", colnames = c("Album", "Track Name", "Release Date"))
```
...
```{r, echo=FALSE, fig.height = 10, fig.weight=10}
